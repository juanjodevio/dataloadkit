...
- [ ] **Step 1:** Set up test infrastructure or document required test resources: **PostgreSQL** (MVP—e.g. Docker `postgres` image), **Redshift** or Redshift-compatible endpoint (optional / separate job if not available in CI), an S3 bucket, an SFTP server (can use a local Docker SFTP container for CI).
- [ ] **Step 2:** Create `tests/integration/conftest.py` with fixtures for: **PostgreSQL** connection string, optional **Redshift** connection string, S3 bucket URL, SFTP URL, test table seeding, cleanup.
- [ ] **Step 3:** Create `tests/integration/test_sql_to_sql.py` — **`Postgres → Postgres`** load (required MVP path); **`Redshift → Redshift`** when infra available; verify rows arrive; test append and replace modes with correct **`sql_dialect`** on source and destination.
...
- [ ] **Step 10:** Create example scripts in `examples/`: `sql_to_sql.py` (show **PostgreSQL** dialect), `sql_to_s3.py`, `s3_to_sql.py`, `to_sftp.py` — each a short, self-contained script using `dlk.from_*().to_*().load()`.
...
- **Requires:** Test infrastructure — **PostgreSQL** (required for MVP CI path), **Redshift** (optional/scheduled), S3 bucket, SFTP server.
...
- **PostgreSQL** is a first-class MVP target (not only a stand-in for Redshift); document any **Redshift-specific** behaviors that differ when both are exercised.
