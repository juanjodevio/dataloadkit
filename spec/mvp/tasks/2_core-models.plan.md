...
- [ ] **Step 8:** Write validation functions in `dlk/core/validation.py` — validate a `LoadPlan` before execution (merge needs PK, destination required, filesystem format valid, incremental needs cursor field, etc.).
