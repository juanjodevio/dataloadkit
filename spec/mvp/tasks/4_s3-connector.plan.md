...
- [ ] Supports explicit file format (CSV, JSON, Parquet).
...
- [ ] **Step 4:** Implement format inference from file extension in `dlk/utils/format.py` (e.g. `.csv` → CSV, `.json`/`.jsonl` → JSON, `.parquet` → Parquet) — used as fallback when format is not explicit.
- [ ] **Step 5:** Handle glob patterns (pass through to dlt filesystem source).
- [ ] **Step 6:** Add unit tests in `tests/connectors/test_s3.py` — mock dlt; assert correct shape for: single file explicit format, glob pattern, inferred format.
- [ ] **Step 7:** Add unit tests in `tests/utils/test_format.py` for format inference.
...
- `dlk/utils/format.py`, `__init__.py`
...
- `uv run pytest tests/connectors/test_s3.py tests/utils/test_format.py` — all pass
